...
<style type="text/css">
	...
	.new-popup{width:99%;float:left;padding:5px;margin-bottom:10px;border:1px solid #e1e1e1;background: #fff;}
	.new-popup .head-title{font-weight:bold;}
	.new-popup tr{}
	.new-popup tr td{padding: 5px 0px;}
	form .bottom-controls{float: left;border: 1px solid #e1e1e1;padding: 20px 6px 10px 6px;width: 99%;background: #fff;}
	form .bottom-controls input{margin-right: 10px!important;margin-bottom: 10px!important;}
...
<div class="wrap">
    ...
    if (isset($_POST["alter_default_settings"])) {
        echo('<h2>Manage RSS Feeds - Default Settings</h2>');
    } else {
        echo('<h2>Manage RSS Feeds</h2>');
    }
    ?>   
    ...
    if (isset($_GET["edit-feed-id"])) {
        ...
        $csyn_syndicator->showMainPage(false);
    } elseif (isset($_POST["new_feed"]) && strlen($_POST["feed_url"][0]) > 0) {
        if ($csyn_syndicator->feedPreview($csyn_syndicator->fixURL($_POST["feed_url"][0]), false)) {
		...
		$feed['feed_urls'] = $_POST['new_feed_urls'];
        $feed['updated'] = 0;
