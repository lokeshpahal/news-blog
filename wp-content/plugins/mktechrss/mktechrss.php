<?php
/*
  Plugin Name: Market Technologies RSS
  Version: 1.01
  Author: Market Technologies
  Author URI: 
  Plugin URI: 
  Description: This is easy to use RSS aggregation and content ciration plugin for WordPress. This can be used to Fetch content from multiple sources at once.
 */
...
class CyberSyn_Syndicator {
	...
	var $completePost = array();
	var $completePostLink;
    ...
    function parseFeed($feed_url) {
        ...
        $this->current_feed_urls = @$_POST["feed_url"];
            ...
            for ($i = 0; $i < $feeds_cnt; $i++) {
                if (in_array($i, $feed_ids)) {
                    if (!$check_time || $this->getUpdateTime($this->feeds [$i]) == "asap") {
						...
						if(!isset($this->current_feed['feed_urls'])){
							if ($this->show_report) {
							...
							$finalPost = $this->mergePosts($this->completePost);
							$post_id = wp_insert_post($finalPost, true);
							add_post_meta($post_id, 'cyberseo_rss_source', $this->current_feed ['url']);
							add_post_meta($post_id, 'cyberseo_post_link', $this->completePostLink);
							...
							$this->completePost = array();
							$this->completePostLink = '';
						}else{
							foreach($this->current_feed['feed_urls'] as $furl){
								$this->resetPost();
								if ($this->show_report) {
									echo 'Syndicating <a href="' . htmlspecialchars($furl) . '" target="_blank"><strong>' . $this->current_feed ['title'] . "</strong></a>...\n";
									flush();
								}
								$this->count = 0;
								$result = $this->parseFeed($furl);
								if ($this->show_report) {
									if ($this->count == 1) {
										echo $this->count . " post was added";
									} else {
	...
	function mergePosts($postsArray=array()){
		$tmpPost = $postsArray[0];
		$postCounter = 0;
		if(!empty($postsArray) && count($postsArray)>0){
			foreach($postsArray as $singlePost){
				$postCounter++;
				if($postCounter>1){
					$tmpPost['post_content'] .= $singlePost['post_content'];
				}
			}
		}
		return $tmpPost;
    ...
    function insertPost() {
		...
		$fullContent = $this->fetchFullContent($this->post['link']);
		
		if($fullContent){
			$this->post['post_content'] = $fullContent;
		}
		
		$this->post['post_content'] = $this->post['post_content'].'<p class="feed-source-link"><a target="_blank" href="'.$this->post['link'].'">This content was first seen on "'.$this->post['post_title'].'".</a></p>';
		$this->post['post_content'] = preg_replace(array('{<a[^>]*><img}','{/></a>}'), array('<img','/>'), $this->post['post_content']);
        ...
        if (mb_strlen($this->post ['post_title'])) {
            ...
            if (!$result_dup) {
				...
				$this->completePostLink = $this->post ['link'];
				/*
                $post_id = wp_insert_post($post, true);
				*/
                if (empty($post) && $this->show_report) {
                    $this->failure = true;
    ...
    function showSettings($islocal, $settings) {
        ...
        echo '<form enctype="multipart/form-data" action="' . preg_replace('/\&edit-feed-id\=[0-9]+/', '', csyn_REQUEST_URI()) . '" method="post">' . "\n";
        ?>
            ...
            <tbody>
                <?php if ($islocal) { ?>
                    ...
                    <tr>
                        <td>Feed URL's</td>
                        <td>
						<?php foreach($this->current_feed_urls as $fuel){
						if($fuel!==''){
						?>
						<input type="text" name="new_feed_url[]" size="132" value="<?php echo htmlspecialchars($fuel); ?>"<?php
							if (!$this->edit_existing) {echo ' disabled'; } ?>>
						<?php } } ?>
                        </td>
                    </tr>
                <?php } ?>
            ...
            echo "Syndicate new feeds to the following categories";
                ...
                <tr>
                    ...
                    <td><select name="insert_media_attachments" size="1">
                            ...
                            ?>
                        </select> - if enabled plugin syndicator will insert media attachments (if available) into the aggregating post. The
        ...
        if ($islocal) {
            if ($this->edit_existing) {
				...
				foreach($this->current_feed_urls as $fuel){
					if($fuel!==''){
						echo '<input type="hidden" name="new_feed_urls[]" value="' . $fuel . '">' . "\n";
					}
				}
            }
        } else {
    ...
    function showMainPage($showsettings = true) {
        ...
        //echo $csyn_banner;
		?>
		<script src="<?php echo plugins_url('jquery.min.js', __FILE__); ?>" type="text/javascript"></script>
		<script>
			$(document).ready(function(){
				var new_feed = '<tr><td align="left"><input type="text" name="feed_url[]" value="" size="100">&nbsp;<input class="button-primary remove-more-feed-url" name="new_feed" value="Remove" type="button"></td></tr>';
				$('.add-more-feed-url').click(function(){
					$('.first-feed-url').after(new_feed);
				});
				$(document).on('click','.remove-more-feed-url',function(){
					$(this).parent().parent().remove();
				});
			});
		</script>
		<?php
        echo '<div class="new-popup"><form action="' . csyn_REQUEST_URI() . '" method="post">' . "\n";
        echo '<table class="form-table" width="100%">';
        echo "<tr class='first-feed-url'><td align=\"left\">\n";
        echo '<div><span class="head-title">New Feed URL: </span></div><div><input type="text" name="feed_url[]" value="" size="100">';
        echo '&nbsp;<input class="button-primary add-more-feed-url" name="new_feed" value="Add More" type="button"></div>' . "\n";
        echo "</td></tr>\n";
        ...
        if (count($this->feeds) > 0) {
            ...
            for ($i = 0; $i < count($this->feeds); $i++) {
				...
				if(isset($this->feeds[$i]['feed_urls'])){
					echo '<td>';
					foreach($this->feeds[$i]['feed_urls'] as $fuls){
						echo '<a href="' . $fuls . '" target="_blank">' . csyn_short_str(htmlspecialchars($fuls), 100) . '</a><br/>';
					}
					echo '</td>';
				}else{
        ...
        <div class="submit">
            <table width="100%">
                <tr>
					<td>
                        <div align="left" class="bottom-controls">
							<input class="button-primary" name="check_for_updates" value="Pull feeds now!" type="submit">
                            <input class="button-primary" name="delete_feeds_and_posts" value="Delete feeds and syndicated posts" type="submit">
                            <input class="button-primary" name="delete_feeds" value="Delete feeds" type="submit">
                            <input class="button-primary" name="delete_posts" value="Delete posts syndycated from feeds" type="submit">
							<input class="button-primary" name="alter_default_settings" value="Default settings" type="submit">
                        </div>
                    </td>
                    <td></td>
                </tr>
	...
	function fetchFullContent($url='',$conLinks=false){
		...
		if($url){
			...
			$readability->convertLinksToFootnotes = $conLinks;
...
function csyn_main_menu() {
    if (function_exists('add_menu_page')) {
        add_menu_page('MktechRss', 'MktechRss', 'add_users', DIRNAME(__FILE__) . '/mktechrss-syndicator.php');
        add_submenu_page(DIRNAME(__FILE__) . '/mktechrss-syndicator.php', 'Options', 'Options', 'add_users', DIRNAME(__FILE__) . '/mktechrss-options.php');
        add_submenu_page(DIRNAME(__FILE__) . '/mktechrss-syndicator.php', 'The Best Spinner Module', 'The Best Spinner', 'add_users', DIRNAME(__FILE__) . '/mktechrss-bs.php');
        add_submenu_page(DIRNAME(__FILE__) . '/mktechrss-syndicator.php', 'WordAi Module', 'WordAi', 'add_users', DIRNAME(__FILE__) . '/mktechrss-wa.php');
    }
}
