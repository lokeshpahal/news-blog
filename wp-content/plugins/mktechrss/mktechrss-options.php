...
<div class="wrap">
    ...
    if (!function_exists('mb_convert_case')) {
        $problems .= "The required <a href=\"http://php.net/manual/en/book.mbstring.php\" target=\"_blank\">mbstring</a> PHP extension is not installed. You must install it in order to make plugin work properly.\n<br />";
    }
    if (!function_exists('curl_init') && ini_get('safe_mode')) {
        $problems .= "PHP variable <a href=\"http://php.net/manual/en/features.safe-mode.php\" target=\"_blank\">safe_mode</a> is enabled. You must disable it in order to make plugin work properly.\n<br />";
    }
    if (!function_exists('curl_init') && !ini_get('allow_url_fopen')) {
        $problems .= "PHP variable <a href=\"http://php.net/manual/en/filesystem.configuration.php#ini.allow-url-fopen\" target=\"_blank\">allow_url_fopen</a> is disabled. You must enable it in order to make plugin work properly.\n<br />";
    }
    if ($problems != "") {
        echo "<div id=\"message\" class=\"error\"><p>$problems</p></div>\n";
    }
    ?>
    <h2>RSS Feed General Settings</h2>
	<div class="main-box">
    ...
    ?></select><br/> set the RSS pulling mode. If you have no access to a crontab, or not sure on how to set a cron job, set the RSS Pull Mode to "auto".
                    <br />

                    <p id="auto">
                        In this mode, the plugin uses WordPress pseudo cron, which will be executed by the WordPress every<br/><input type="text" name="<?php echo CSYN_PSEUDO_CRON_INTERVAL; ?>" size="1" value="<?php echo get_option(CSYN_PSEUDO_CRON_INTERVAL); ?>"><br/>minutes. 
                        The pseudo cron will trigger when someone visits your WordPress site, if the scheduled time has passed.
                    </p>

                    <p id="cron">
                        In this mode, you need to manually configure <strong><a href="http://en.wikipedia.org/wiki/Cron" target="_blank">cron</a></strong> at your host. For example, if you want run a cron job once a hour, just add the following line into your crontab:<br/><br/>
            ...
            <tr valign="top">
                ...
                <td align="left"><input type="checkbox" name="<?php echo CSYN_DISABLE_DUPLICATION_CONTROL; ?>"
                    ...
                    ?> />
                    allows the plugin to syndicate a same feed more than once.
                </td>
            </tr>
